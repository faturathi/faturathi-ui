  ...
  const dialog = request && createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) close(false); }}>
      <div role="alertdialog" aria-modal="true" aria-labelledby="critical-confirm-title" className="w-full max-w-lg overflow-hidden rounded-3xl border border-white/20 bg-white shadow-2xl animate-fadeIn">
        <div className={`flex items-start justify-between gap-4 p-5 text-white ${request.kind === 'submit' || request.kind === 'danger' ? 'bg-gradient-to-r from-amber-700 to-red-700' : 'bg-gradient-to-r from-[#0d4f8b] to-[#0b7a63]'}`}>
          <div className="flex gap-3">
            <div className="rounded-2xl border border-white/25 bg-white/15 p-2.5">{request.kind === 'submit' || request.kind === 'danger' ? <ShieldAlert className="h-6 w-6" /> : request.title.toLowerCase().includes('user') ? <UserPlus className="h-6 w-6" /> : <Building2 className="h-6 w-6" />}</div>
        ...
        <div className="space-y-4 p-6">
          ...
          {request.detail && <div className={`flex items-start gap-2.5 rounded-2xl border p-3.5 text-xs leading-5 ${request.kind === 'submit' || request.kind === 'danger' ? 'border-red-200 bg-red-50 text-red-900' : 'border-amber-200 bg-amber-50 text-amber-950'}`}><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span>{request.detail}</span></div>}
          <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
            ...
            <button type="button" onClick={() => close(true)} className={`inline-flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-xs font-bold text-white shadow-md ${request.kind === 'submit' || request.kind === 'danger' ? 'bg-red-700 hover:bg-red-800' : 'bg-[#0d4f8b] hover:bg-blue-900'}`}><FileCheck2 className="h-4 w-4" />{request.confirmLabel}</button>
          </div>
        </div>
      </div>
    </div>, document.body);
